...
Specifies the maximum time an activity or a task can run before it times out with the [States.Timeout](concepts-error-handling.md#statestimeout) error and fails. The timeout value must be positive, non-zero integer. The default value is 99,999,999.  
